...
: D. Michael Ackermann
: Created 6/23/08
: Filename = AXNODEKExt.mod
: Version = 1.0
: Desc = Node dynamics Mod file for implementing EC K+ accumulation in MRG model in the extranodal space
: Modified from original MRF AXNODE.mod file (see below)
: Changes = changed nonspecific_current ik to useion k, read ek write ik
...
NEURON {
	...
	: NONSPECIFIC_CURRENT ik   :Replaced with below to allow KExtNode to track ik
	USEION k READ ek WRITE ik    :Assign the K+ input/ouputs so KExtNode can interface with the K+ dynamics
...
PARAMETER {
	...
	gl	= 0.007 (mho/cm2)
	ena     = 50.0  (mV)
	: ek      = -90.0 (mV)     :commented out, because ek is now read into the mod file (declared in Assigned instead)
	el	= -90.0 (mV)
	celsius		(degC)
	dt              (ms)
	v               (mV)
...
ASSIGNED {
	ek (mV)  :Added and assigned here in Assigned space with units mV
	inap    (mA/cm2)
	ina	(mA/cm2)
	ik      (mA/cm2)
	il      (mA/cm2)
