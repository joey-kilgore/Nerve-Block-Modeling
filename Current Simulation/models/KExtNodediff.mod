...
NEURON {
  SUFFIX KExtNodediff      :Suffix tells Neuron that this is a Density Mechanism, with Suffix KExtNode
...
PARAMETER {    :Parameters = constants
  ...
  txfer = 100 : time constant for diffusion
}
...
DERIVATIVE state {
   ko' = (1e4)*(ik*N_SA)/(FARADAY*NSh_vol) - (ko - ko0)/txfer
}   